...
You will be using Istio version 1.20.2:
...
Note the istio distribution folder `istio-1.20.2` in your home directory:
...
```
cp istio-1.20.2/bin/istioctl /usr/local/bin
```{{exec}}
...
Note how the output of the above command states in so many words that Istio is not yet installed in your Kubernetes cluster, but that the client version is 1.20.2.

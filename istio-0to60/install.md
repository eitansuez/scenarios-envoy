...
You will be using Istio version 1.19.0:
...
Note the istio distribution folder `istio-1.19.0` in your home directory:
...
```
cp istio-1.19.0/bin/istioctl /usr/local/bin
```{{exec}}
...
Note how the output of the above command states in so many words that Istio is not yet installed in your Kubernetes cluster, but that the client version is 1.19.0.
